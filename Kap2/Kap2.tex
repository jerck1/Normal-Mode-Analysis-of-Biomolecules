...
V=\left\{\begin{array}{cl}
   \frac{1}{2}\sum_{i\neq j=1}^{N}\gamma_{ij}\left(R_{ij}-R_{ij}^0\right)^2 &\mbox{  si } R_{ij},R_{ij}^0<R_c\\
   0 &\mbox{  Otro caso}
  \end{array}
\right.
\end{equation}\label{eq:26}
Donde $R_{ij}^0$ es la distancia de equilibrio entre los nodos $i-j$ de la red, la cual  $R_{ij}$ su distancia variable y $\gamma_{ij}$ las constantes el\'{a}sticas entre los nodos $i-j$. Como se ilustra en la figura \ref{fig:res}, el t\'{e}rmino $R_{ij}-R_{ij}^0$ es la elongaci\'{o}n del resorte, lo cual es diferente a la resta de los desplazamientos, entonces no se est\'{a} aproximando el potencial, por tanto no se requiere una minimizaci\'{o}n de la energ\'{i}a, como se ha recalcado, s\'{o}lo se cambia la forma del potencial.\\
\begin{figure}
\centering%
...
\end{equation}\label{eq:34}
Los elementos de $\mathbf{H}_{ij}$ son:
\begin{equation}
\mathbf{H}_{ij}=\left(
\begin{array}{ccc}
 \partial_x\partial_x V & \partial_x\partial_y V & \partial_x\partial_z V \\
 \partial_y\partial_x V & \partial_y\partial_y V & \partial_y\partial_z V \\
 \partial_z\partial_x V & \partial_z\partial_y V & \partial_z\partial_z V
\end{array}
\right)
\end{equation}\label{eq:35}

Calculando algunas de estas derivadas y definiendo $x_{ik}=x_k-x_i$, $y_{ik}=y_k-y_i$ y $z_{ik}=z_k-z_i$ se obtiene:
\begin{equation}
\frac{\partial V }{\partial y_k}=\sum_j\frac{\gamma_{jk}\left(R_{kj}-R_{kj}^0\right)y_{kj}}{R_{kj}}
\end{equation}\label{eq:36}
\begin{equation}
\frac{\partial^2 V }{\partial x_i\partial y_j}\bigg|_{R_{ij}=R_{ij}^0}=-\frac{\gamma_{ij}x_{ij}y_{ij}}{R_{ij}^2}
\end{equation}\label{eq:37}
\begin{equation}
\frac{\partial^2 V }{\partial x_i\partial x_j}\bigg|_{R_{ij}=R_{ij}^0}=-\frac{\gamma_{ij}x_{ij}^2}{R_{ij}^2}
\end{equation}\label{eq:38}
Para las submatrices que se encuentran en la diagonal:
\begin{equation}
\frac{\partial^2 V }{\partial x_i^2}\bigg|_{R_{ij}=R_{ij}^0}=-\sum_{j\neq i}\gamma_{ij}\left(.\right)
\end{equation}\label{eq:39}
...
\begin{equation}
V=\frac{\gamma}{2}\sum_{i,j=1}^{N}\left(\textbf{R}_{ij}-\textbf{R}_{ij}^0\right)^2
