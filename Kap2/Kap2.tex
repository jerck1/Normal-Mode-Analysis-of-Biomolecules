...
Ct_i=\lambda_it_i
\end{equation*}
Tomando $\Lambda={\lambda_i}$, la matriz diagonal formada por los autovalores a y $T=\left(t_1, t_2,...,t_3N\right)$ como la matriz cuyas columnas son los autovectores entonces:
\begin{equation}
\Lambda=T^TCT
\end{equation}\label{eq:29}
Donde se ha tenido en cuenta que $T^{-1}=T^T$. De forma equivalente:
\begin{equation}
C=T\Lambda T^T
