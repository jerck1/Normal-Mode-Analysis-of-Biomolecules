...
Los ENM, como la palabra \textit{el\'{a}stico} lo indica, se basan en una simplificaci\'{o}n de la energ\'{i}a potencial a una energ\'{i}a potencial el\'{a}stica, es decir de tipo Hooke. Un requisito para que sea posible hacer dicha simplificaci\'{o}n, es el hecho de que la energ\'{i}a potencial sea minimizable. La diferencia con el NMA est\'{a}ndar es que ya no se toman todos los \'{a}tomos en el modelo, sino los \textit{residuos} de la biomol\'{e}cula. Por ejemplo en una prote\'{i}na los residuos son sus amino\'{a}cidos.\\

Al simplificar el potencial, la biomol\'{e}cula original se convierte en una red cuyos nodos est\'{a}n sometidos al potencial el\'{a}stico, ver figura \ref{fig:pan}. Los nodos se consideran como bloques constituyentes de la biomol\'{e}cula. Aunque el bloques constituyentes suelen ser los residuos de la biomol\'{e}cula, puede variar ligeramente para dar compatibilidad al modelo con los datos experimentales. Por ejemplo en las prote\'{i}nas como la BPTI, ver \cite{Gur2013}, los constituyentes son los carbonos $\alpha$ de los amino\'{a}cidos.\\\\

El modelo es bueno cuando hay una buena estabilidad de los enlaces con respecto a su posici\'{o}n de equilibrio; para distancias mucho mayores que lo que se donomina la distancia de corte $R_c$ no hay potencial alguno. La distancia de corte es uno de los par\'{a}metros requeridos en el modelo, otro son las constantes el\'{a}sticas.\\
...
\section{Descripci\'{o}n Mec\'{a}nica del Modelo}

Consid\'{e}rese una biomol\'{e}cula con $N$ part\'{i}culas constituyentes, la energ\'{i}a potencial $V$ que representa las interacciones entre los constituyentes de la biomol\'{e}cula, se puede expresar alrededor de las posiciones de equilibrio $\mathbf{q_0}=\mathbf{0}$ tal como describe la teor\'{i}a de peque\~{n}as oscilaciones, ver \cite{Goldstein2001}:
\begin{equation}
V(\mathbf{q})=V(\mathbf{0})+\sum_{i=1}^n\frac{\partial V}{\partial q_i}q_i+\frac{1}{2}\sum_{ij}^{n}\frac{\partial^2 V }{\partial q_i\partial q_j}q_i q_j+...
...
\begin{equation}
V(\mathbf{q})=\frac{1}{2}\sum_{i,j=1}^{n}\frac{\partial^2 V }{\partial q_i\partial q_j}q_i q_j
...
Tomando $\hbar=1$ y teniendo en cuenta que el determinante de $\mathbf{K}$ es el producto de los autovalores, es decir, de las frecuencias al cuadrado:
\begin{eqnarray}
...
Es un modelo en el que se considera un potencial con la diferencia de distancias y no con los desplazamientos alrededor del equilibrio. Se requieren dos par\'{a}metros para describir el modelo: Una misma constante el\'{a}stica para todas las unidades de la biomol\'{e}cula y una distancia de corte a partir de la cual no se ejerce un potencial.
