\chapter{Estudios del Cotransportador vSGLT}\label{ch:3}
...
\put(-70,0){\textbf{(a)}}
\includegraphics[scale=0.2]{Kap3/hemo.png}
\put(-70,0){\textbf{(b)}}
...
\caption{Membranas interna y externa de una bacteria gram negativa mostrando algunas de las macromol\'{e}culas que la constituyen. En el medio de las dos membranas est\'{a} la pared celular que divide el periplasma en dos partes. Traducida de \cite{Dahl2008File:Gram-}}\label{fig:cell}
\end{figure}
...
La estructura tridimensional de los prote\'{i}nas de membrana, como se ha mencionado, puede ser obtenida mediante cristalograf\'{i}a de rayos x. Para obtener un buen patr\'{o}n de estructura cristalina, es deseable separar la prote\'{i}na de membrana de la bicapa lip\'{i}dica, pero al hacer esto la prote\'{i}na pierde su estructura porque la forma de la prote\'{i}na est\'{a} determinada, en parte, por las interacciones hidrof\'{o}bicas con los fosfol\'{i}pidos de la bicapa \cite{Editor2010}. Adicionalmente, al extraerla puede perder su funci\'{o}n biol\'{o}gica dentro de la membrana.\\

Para poder extraer las prote\'{i}nas integrales de membrana se suelen usar soluciones detergentes que rodean la prote\'{i}na volvi\'{e}ndolas liposolubles, ver figura \ref{fig:mem_prot}. Las soluciones detergentes se escogen de tal manera que la prote\'{i}na de membrana conserve su estructura y su funci\'{o}n biol\'{o}gica adem\'{a}s de permitir el estudio bioqu\'{i}mico de estas dos propiedades \cite{Editor2010}.\\
\begin{figure}[h]
 \centering
 \includegraphics[scale=0.3]{Kap3/mem_prot.png}
 % mem_prot.png: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=
 \caption{Purificaci\'{o}n de distintos tipos de prote\'{i}nas de membrana de acuerdo a su ubicaci\'{o}n en la bicapa lip\'{i}dica. Imagen obtenida de \cite{Nelson2011}.}
 \label{fig:mem_prot}
\end{figure}
...
En la base de datos del PDB, se encuentra la estructura de vSGLT  como un tetr\'{a}mero con el c\'{o}digo PDB 3DH4. La estructura tetram\'{e}rica es la de la celda unitaria. Mientras que en su ensamblaje biol\'{o}gico forma un d\'{i}mero, ver figura \ref{fig:complejo}.
\begin{figure}[H]
\centering
...
Los residuos clave, que coordinan con la galactosa son, de acuerdo a \cite{Faham2008}:\\

Y263, W264 del segmento TM7E, M73 del segmento TM2, Y87 del segmento TM7, F424 del segmento TM11, Y263 del segmento TM7, Q428 del segmento TM11, Q69 del segmento TM2, K294 del segmento TM8 y S91 del segmento TM3\\
\begin{figure}[h]
 \centering
 \includegraphics[scale=0.2]{Kap3/gal_binding_site.png}
 % gal_binding_site.png: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=
 \caption{Algunos residuos que interact\'{u}an con la galactosa}
 \label{fig:gal}
\end{figure}
\begin{figure}[h]
 \centering
 \includegraphics[scale=0.2]{Kap3/Na_binding_site.png}
 % gal_binding_site.png: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=
 \caption{Algunos residuos que interact\'{u}an con el sodio}
 \label{fig:Na}
\end{figure}
...
De acuerdo a estudios experimentales \cite{Veenstra2004LocalCotransporter.} y \cite{Adelman2016}, en el espacio extracelular de la membrana el sodio se une antes que la galactosa al cotransportador. Sin embargo, el ingreso de los solutos hacia el espacio intermembranal a\'{u}n no ha sido aclarado.\\

La estructura del mutante K294A mostrada en la figura \ref{fig:2xq2}, junto con una nueva caracterizaci\'{o}n bioqu\'{i}mica y simulaciones por din\'{a}mica molecular, todas encontradas en \cite{Watanabe2010} muestran que cuando sale el sodio hacia el espacio intracelular, la h\'{e}lice 2 (tomada en la figura \ref{fig:h2xq23dh4} como h\'{e}lice 1) se reorienta, permitiendo que se abra la compuerta para que salga posteriormente la galactosa. Al realizar las simulaciones por din\'{a}mica molecular se encuentra que a los $\sim 52$ns la Tyr263 hace su transici\'{o}n entre dos rot\'{a}meros para dejar pasar la galactosa al espacio intracelular,  tal como se muestra en la figura \ref{fig:Y263}.\\

Adicionalmente, las simulaciones MD de \cite{Watanabe2010} encontr\'{o} que el tiempo de ingreso del \ce{Na^{+}} al espacio intermembranal es de $9$ns, mientras que el de la galactosa es de $110$ns. Es decir el tiempo de ingreso de la galactosa es mucho mayor que el tiempo que dura el \ce{Na^{+}} unido al simportador.\\
\begin{figure}[h]
 \centering
 \includegraphics[scale=0.5]{Kap3/helice2.png}
 % helice2.png: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=
 \caption{Transici\'{o}n entre el estado de conformaci\'{o}n hacia adentro ocluido (gris) y el abierto (colores). Figura tomada de \cite{Watanabe2010}}
 \label{fig:h2xq23dh4}
\end{figure}
\begin{figure}[h]
\centering%
\includegraphics[scale=0.3]{Kap3/Y263.png}%
...
\end{figure}
Posteriormente en \cite{Adelman2016} encontraron por medio de 21 simulaciones MD y modelos de estado de Markov, que no existe un orden de entrada para el sodio y la galactosa al espacio intracelular, es decir, es aleatorio. M\'{a}s a\'{u}n, de las 21 MD que fueron realizadas con tiempos de simulaci\'{o}n entre 100 y 500 ns, se encontr\'{o} que (Ver figura ) en 4 de \'{e}stas la galactosa  sale antes que el \ce{Na^{+}} y en una salen al mismo tiempo. En otras se encontr\'{o} que habiendo saldio el \ce{Na^{+}}, la galactosa no sal\'{i}a antes de los tiempos de simulaci\'{o}n. Aunque en 10 situaciones el \ce{Na^{+}} sale antes que la galactosa los tiempos de salida del \ce{Na^{+}} son del orden de los $\sim 100$ns que son dos \'{o}rdenes de magnitud mayores a los encontrados anteriormente en \cite{Watanabe2010}  .\\
\begin{figure}
 \centering
 \includegraphics[scale=0.3]{Kap3/exit_t.png}
 % exit_t.png: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=
 \caption{Tiempos en que el \ce{Na^{+}} y la galactosa permanecen unidos al cotransportador vSGLT. Cada punto corresponde a una simulaci\'{o}n de las 21 realizadas. La flecha hacia arriba indica que la galactosa nunca se desprendi\'{o} del cotransportador durante la simulaci\'{o}n, la flecha horizontal que el \ce{Na^{+}} nunca se desprendi\'{o} durante la simulaci\'{o}n. Figura tomada de \cite{Adelman2016}}
 \label{fig:exit}
\end{figure}

En \cite{Adelman2016} se sugiere que al no verse afectada la rotaci\'{o}n del residuo Tyr263 por la ocupaci\'{o}n del \ce{Na^{+}} en el sitio Na2 \footnote{Nombre dado al sitio que ocupa el sodio por la similitud con los transportadores que comparten el plegamiento de LeuT} y al ser posible el ingreso de la galactosa antes que el \ce{Na^{+}}, entonces la coordinaci\'{o}n entre el \ce{Na^{+}} y la galactosa es baja y \textit{los cambios globales en la conformaci\'{o}n de la prote\'{i}na son bajos}.\\

Tambi\'{e}n se realizan simulaciones por MD incluyendo le h\'{e}lice 1, resuelta en la estructura m\'{a}s reciente de vSGLT, figura \ref{fig:2xq2}. Se encuentra que la h\'{e}lice adicional no es suficiente para aumentar la estabilidad del \ce{Na^{+}} en la estructura.\\